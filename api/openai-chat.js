...
### 1. 정기예금 — 비금융투자(예금) 대표
일정 기간 자금 예치 후 만기 시 원금+약정이자 지급. 예금자보호 1억원(2025.9.1 상향). 이자소득세 15.4%. 중도해지 시 낮은 이율 적용. 만기보유 금융자산 분류.
...
### 10. 생명보험 — 보험 대표
생명 관련 위험 보장. 예금자보호 1억원(2025.9.1 상향). 10년 유지 시 보험차익 비과세. 중도해지 시 환급금 적음.

## 핵심 개념 (큰 그림 — 강의 핵심, 시험 범위)

### 두 가지 기준으로 모든 상품 분류
① 원금보장? → 비금융투자상품(예금·적금·보험) vs 금융투자상품(투자성=원금손실 가능)
② 손실이 원금을 넘나? → 증권(손실=원금 한도) vs 파생(원금 초과 손실 가능)
증권 4갈래: 지분(주식)·채무(채권)·수익(펀드/신탁)·파생결합(ETN/ELS/ELW). 파생: 선물·옵션.
채권 스펙트럼(무위험·단기→신용·유동화): 콜·재정증권·통안증권·CD·국채·기업어음·회사채·MBS·CDO.

### 증권의 4요소 (왜 '증권'인가)
① 금전 투입 ② 공동사업 ③ 타인의 노력 ④ 이익 기대. 성과가 타인의 노력에 달려 투자자가 결과를 직접 통제하지 못함.

### 왜 규제하나 — 정보비대칭
정보비대칭 → 투자 전 역선택(逆選擇), 투자 후 도덕적 해이 → 정부가 증권으로 범주화하고 정보공개·상세설명·숙려제도로 폐해를 줄임.

### 두 원칙 (자주 혼동)
적합성 원칙(Suitability): 판매자가 권유할 때 위험 고지. 적정성 원칙(Appropriateness): 투자자가 스스로 고난도 상품 선택 시 위험 고지. 주도권이 판매자면 적합성, 투자자면 적정성.
...
### 최신 제도
예금자보호 한도 5천만원→1억원(2025.9.1). 배당소득 분리과세 시행(2026~, 한시): 주주환원 우수기업(배당성향 요건 충족) 배당을 종합과세 대신 구간별로 분리과세. 미국주식 결제 T+1, 넥스트레이드(NXT) 출범, 금투세 폐지.
...
8. 4요소·정보비대칭·역선택·도덕적해이·적합성/적정성·분류 밖 서비스(공매도·차익거래·페어트레이딩·대차·자문·중개) 등 큰 개념 질문에는 위 '핵심 개념'을 바탕으로 설명하고, 특정 상품/카테고리가 아니면 action은 "none"`;
}
